...
# This file is a linter script that checks for common style issues in the codebase. It's
# taken from Dolphin, and lightly modified to fix ShellCheck errors.
...
for f in ${modified_files}; do
  ...
  if ! echo "${f}" | grep -eq "[.](cpp|h|mm)$"; then
    continue
  fi
  if ! echo "${f}" | grep -eq "^Source"; then
    continue
  fi
